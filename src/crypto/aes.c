...
#if defined(_CC_PADLOCK_C) && (defined(_CC_HAVE_X86) || defined(_CC_PADLOCK_ALIGN16))
static int aes_padlock_ace = -1;
#endif

#if defined(_CC_AES_ROM_TABLES_)
/*
 * Forward S-box
 */
static const byte_t FSB[256] = {
...
#else /* _CC_AES_ROM_TABLES_ */

/*
 * Forward S-box & tables
 */
static byte_t FSB[256];
...
_CC_API_PUBLIC(int) _cc_aes_crypt_cbc(_cc_aes_t *ctx, int mode, const byte_t *input, size_t length, byte_t iv[16], byte_t *output) {
    ...
    if (mode == _CC_AES_DECRYPT_) {
        while (length > 0) {
            memcpy(temp, input, 16);
            _cc_aes_decrypt(ctx, input, output);
...
_CC_API_PUBLIC(int) _cc_aes_crypt_ctr(_cc_aes_t *ctx, const byte_t *input, size_t length, size_t *nc_off, byte_t nonce_counter[16],
    ...
    while (length--) {
        if (n == 0) {
            _cc_aes_encrypt(ctx, nonce_counter, stream_block);
        ...
        c = *input++;
        *output++ = (byte_t)(c ^ stream_block[n]);
