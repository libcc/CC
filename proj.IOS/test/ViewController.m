...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message{
    ...
    //JS调用OC
    if([message.name isEqualToString:@"JSBridgeDownload"]){

    } else if ([message.name isEqualToString:@"log"]) {
        NSLog(@"%@", message.body);
    }
}
-(WKUserScript*)inject:(NSString *)path {
    NSData *reader = [NSData dataWithContentsOfFile:path];
    NSString *sString = [[NSString alloc] initWithData:reader encoding:NSUTF8StringEncoding];
    //用于进行JavaScript注入
    return [[WKUserScript alloc] initWithSource:sString injectionTime:WKUserScriptInjectionTimeAtDocumentEnd forMainFrameOnly:YES];
}
- (WKWebView *)webView{
    if(_webView == nil){
        ...
        [wkUController addScriptMessageHandler:weakScriptMessageDelegate name:@"JSBridgeDownload"];
        [wkUController addScriptMessageHandler:weakScriptMessageDelegate name:@"log"];
